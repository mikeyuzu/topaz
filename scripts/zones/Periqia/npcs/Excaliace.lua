...
entity.onSpawn = function(npc)
    npc:initNpcAi()
    npc:pathThrough(path[0][0], PATHFLAG_REPEAT)
...
entity.onPath = function(npc)
    ...
    local mobChatMessage = npc:getLocalVar("mobChatMessage") -- 0 = No Msg Sent, 1 = Msg Sent
    ...
    if pathProgressMask > 0 and missionActive == 1 then
        ...
        for _, enemys in pairs(mobs) do
            if npc:checkDistance(enemys) < 12 and npc:getID() ~= enemys:getID() and enemys:isSpawned() then
                if mobChatMessage == 0 then
                    npc:setLocalVar("runTimer", os.time() + math.random(20,40))
                    npc:setLocalVar("mobChatMessage", 1)
        ...
        for _, players in pairs(chars) do
            if npc:checkDistance(players) < 5 and npc:isFacing(players) and players:isFacing(npc) then
                rangeStop = true
            end
            if npc:checkDistance(players) < 4 then
                rangeClose = true
            end
            --if npc:checkDistance(players) < 10 and players:isFacing(npc) then -- check if at least 1 player is tailing the NPC
            if npc:checkDistance(players) < 10 then -- ƒvƒŒƒCƒ„[‚Ì•ûŒü‚Í–³Ž‹
                rangeFollow = true
            end
